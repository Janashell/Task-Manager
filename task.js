...
// Template Literal allows us to create seperate lines
...
// Loops and continues to display the  menu until the user ends the program (AKA Until the user enters "CLOSE" Uppercase and Lowercase)
while (userInput !== "CLOSE") {
    ...
    if (userInput === "TASKS") {
        ...
        showTasks = "";
        // Checks to see if user entered "NEW". Uppercase or lowercase
    } else if (userInput === "NEW") {
        // Prompts the user to enter a new task and stores their response
        newTask = prompt(`Please enter the new task:`);

        // Loop that continues looping until the user enters something
        while (true) {
            ...
            if (newTask === "") {
                ...
                // Breaks out of the loop since the user enters something
                break;
            }
        ...
        while (true) {
            // Concatenates each task/item in the tasks array to the showTasks strubg variable. Also, sets a number for each task.
            for (i = 0; i < tasks.length; i++) {
                //  Using \n to create a new line. Also, adding 1 to i (AKA The index) so the numbers will start displaying at 1.
                showTasks += `${i + 1}: ${tasks[i]}\n`;
            }
            // Prompts the user to enter a number and stores their response. Also, using \n to create a new line.  Finally, subracting 1 from the user's entry so it matches the index in the tasks array. (NOTE: JavaScript will attempt to convert a string into a number when you try to subtract from it. Just like ParseInt. So ParseInt is not actually necessary here.)
